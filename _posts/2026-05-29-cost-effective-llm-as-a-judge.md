...
I had been using a set of these techniques to improve evaluation on real customer data; for the paper we consolidated them and ran standardised experiments on RewardBench 2 (paper and code above). We tested four: ensembling, a one-line task-specific criterion, a calibration example to fight anchoring, and adaptive escalation to a bigger model when the cheap one looked unsure. The two simplest, ensembling and criteria, did almost all the work, taking the full-size model from roughly 72% to 84%, and the more complex methods were dominated once you priced them in. One thing I would change: we ran the sweep starting with the large model, so I only realised late that the simple methods dominated, where doing the cheap end first would have saved the effort I spent on the complex approaches.
...
That reframing is what I am working on next. Ensembling the same model averages away noise but not bias, because every sample shares the model's blind spots. Decomposing the error in the ensemble's outputs should give a better handle on uncertainty, in the spirit of AISI's [Skewed Score](https://www.aisi.gov.uk/blog/llm-judges-on-trial-a-new-statistical-framework-to-assess-autograders), and might rescue the model-routing idea that underperformed here. The techniques also need testing on agentic, long-context tasks: RewardBench 2 averages 576 tokens per example, a long way from a real workload.

The version I find most interesting raises the stakes. The moment you trust one of these cheap consistency or variance checks as a detector, say to flag a model underperforming on purpose, the same bias and variance picture tells you when it fails. A model that sandbags only when it senses an eval can move its tell out of the bias a detector sees and into the variance it cannot, so the check reads clean exactly when it should not. AISI's [auditing-games work](https://arxiv.org/abs/2512.07810) has shown black-box detection beaten by this kind of conditional sandbagger, but only qualitatively. I am now pointing the bias and variance decomposition from [The Hot Mess of AI](https://arxiv.org/abs/2601.23045) at it, to work out when these detectors give false reassurance and whether the gap can be closed without already knowing the trigger. That is the next post.
