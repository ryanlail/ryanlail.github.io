...
I had been using a set of these techniques to improve evaluation on real customer data; for the paper we consolidated them and ran standardised experiments on RewardBench 2 (paper and code above). We tested four: ensembling, a one-line task-specific criterion, a calibration example to fight anchoring, and adaptive escalation to a bigger model when the cheap one looked unsure. The two simplest, ensembling and criteria, did almost all the work, taking accuracy from 72.3% to 85.8%, and the more complex methods were dominated once you priced them in. One thing I would change: we ran the sweep starting with the large model, so I only realised late that the simple methods dominated, where doing the cheap end first would have saved the effort I spent on the complex approaches.
